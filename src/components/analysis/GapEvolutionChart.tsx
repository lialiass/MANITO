...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div className="bg-[#0d1526] border border-[#1e3560] rounded-xl px-3 py-2 text-xs shadow-xl">
      ...
      <p className={`font-bold ${color}`}>{sign}{minutesToReadable(val)}</p>
    </div>
  )
}
    ...
    <div className="bg-[#0e1628] border border-[#162030] rounded-2xl p-4">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <SquareLegend color="#10b981bf" label="Temps disponible" />
          <SquareLegend color="#ef4444bf" label="Surplus de temps" />
        </div>
      </div>
